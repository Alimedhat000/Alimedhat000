...
def image_to_ascii(
    ...
    # Resize alpha channel to match final dimensions
    alpha_resized = Image.fromarray(alpha).resize((image_width, image_height))
    transparent_mask = np.array(alpha_resized) < 128

    # Convert image to ASCII characters
    ascii_converted = np.array([charset[i] for i in image_array.flatten()])
